...
<div class="container mx-auto p-4">
    ...
    <form method="POST" action="{{ route('pembelian.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <input id="jumlah" type="number" name="jumlah" value="{{ old('jumlah') }}" required class="border rounded w-full p-2 @error('jumlah') border-red-500 @enderror">
            @error('jumlah')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            ...
            <input id="tanggal" type="date" name="tanggal" value="{{ old('tanggal') }}" required class="border rounded w-full p-2 @error('tanggal') border-red-500 @enderror">
            @error('tanggal')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
